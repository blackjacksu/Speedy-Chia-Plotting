...
struct BufferedDisk : Disk
{
    ...
    uint8_t const* Read(uint64_t begin, uint64_t length);

    void Write(uint64_t const begin, const uint8_t *memcache, uint64_t const length);

    void Truncate(uint64_t const new_size);
