...
uint8_t const* SortManager::Read(uint64_t begin, uint64_t length) override
{
    assert(length <= entry_size_);
    return ReadEntry(begin);
}
void SortManager::Write(uint64_t, uint8_t const*, uint64_t) override
{
    assert(false);
    throw InvalidStateException("Invalid Write() called on SortManager");
}
void SortManager::Truncate(uint64_t new_size) override
{
    if (new_size != 0) {
        assert(false);
...
std::string SortManager::GetFileName() override
{
    return "<SortManager>";
}
void SortManager::FreeMemory() override
{
    for (auto& b : buckets_) {
        b.file.FreeMemory();
