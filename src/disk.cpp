...
explicit FileDisk::FileDisk(const fs::path &filename)
{
    filename_ = filename;
    Open(writeFlag);
}
...
FileDisk::FileDisk(const FileDisk &) = delete;
FileDisk::FileDisk &operator=(const FileDisk &) = delete;
void FileDisk::Close()
{
...
uint64_t FileDisk::GetWriteMax() const noexcept { return writeMax; }
...
uint8_t const* BufferedDisk::Read(uint64_t begin, uint64_t length) override
{
...
void BufferedDisk::Write(uint64_t const begin, const uint8_t *memcache, uint64_t const length) override
{
    NeedWriteCache();
...
void BufferedDisk::Truncate(uint64_t const new_size) override
{
    FlushCache();
...
std::string BufferedDisk::GetFileName() override { return disk_->GetFileName(); }
void BufferedDisk::FreeMemory() override
{
    FlushCache();
    read_buffer_.reset();
    write_buffer_.reset();
...
uint8_t const* FilteredDisk::Read(uint64_t begin, uint64_t length) override
{
...
void FilteredDisk::Write(uint64_t begin, const uint8_t *memcache, uint64_t length) override
{
    assert(false);
    throw std::runtime_error("Write() called on read-only disk abstraction");
}
void FilteredDisk::Truncate(uint64_t new_size) override
{
    underlying_.Truncate(new_size);
    if (new_size == 0) filter_.free_memory();
}
std::string FilteredDisk::GetFileName() override { return underlying_.GetFileName(); }
void FilteredDisk::FreeMemory() override
{
    filter_.free_memory();
