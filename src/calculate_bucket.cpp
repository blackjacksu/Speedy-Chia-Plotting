...
F1Calculator::F1Calculator() = default;

inline F1Calculator::F1Calculator(uint8_t k, const uint8_t* orig_key)
{
    uint8_t enc_key[32];
...
inline F1Calculator::~F1Calculator()
{
    delete[] buf_;
}
// Disable copying
F1Calculator::F1Calculator(const F1Calculator&) = delete;
// Reloading the encryption key is a no-op since encryption state is local.
inline void F1Calculator::ReloadKey() {}
// Performs one evaluation of the F function on input L of k bits.
inline Bits F1Calculator::CalculateF(const Bits& L) const
{
...
inline std::pair<Bits, Bits> F1Calculator::CalculateBucket(const Bits& L) const
{
...
FxCalculator::FxCalculator() = default;
inline FxCalculator::FxCalculator(uint8_t k, uint8_t table_index)
{
    this->k_ = k;
...
inline FxCalculator::~FxCalculator() = default;
// Disable copying
FxCalculator::FxCalculator(const FxCalculator&) = delete;
inline void FxCalculator::ReloadKey() {}
// Performs one evaluation of the f function.
inline std::pair<Bits, Bits> FxCalculator::CalculateBucket(const Bits& y1, const Bits& L, const Bits& R) const
{
    Bits input;
...
inline int32_t FxCalculator::FindMatches(
