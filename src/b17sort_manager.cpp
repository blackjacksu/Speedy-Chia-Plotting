...
#include "../include/b17sort_manager.h"
...
inline void b17SortManager::AddToCache(const Bits &entry)
{
    entry.ToBytes(this->entry_buf);
    return AddToCache(this->entry_buf);
}

inline void b17SortManager::AddToCache(const uint8_t *entry)
{
    if (this->done) {
...
inline uint8_t * b17SortManager::ReadEntry(uint64_t position, int quicksort = 0)
{
...
inline bool b17SortManager::CloseToNewBucket(uint64_t position) const
{
...
inline void b17SortManager::TriggerNewBucket(uint64_t position, bool quicksort)
{
    if (!(position <= this->final_position_end)) {
...
inline void b17SortManager::ChangeMemory(uint8_t *new_memory, uint64_t new_memory_size)
{
    this->FlushCache();
...
inline void b17SortManager::FlushCache()
{
    for (size_t bucket_i = 0; bucket_i < this->mem_bucket_pointers.size(); bucket_i++) {
        FlushTable(bucket_i);
    }
...
inline void b17SortManager::FlushTable(uint16_t bucket_i)
{
...
inline void b17SortManager::SortBucket(int quicksort)
{
    this->done = true;
