...
explicit DiskProver::DiskProver(const std::string& filename) : id(kIdLen)
{
...
const std::vector<uint8_t>& DiskProver::GetMemo() { return memo; }
const std::vector<uint8_t>& DiskProver::GetId() { return id; }
std::string DiskProver::GetFilename() const noexcept { return filename; }
...
LargeBits DiskProver::GetFullProof(const uint8_t* challenge, uint32_t index, bool parallel_read = true)
{
...
void DiskProver::SafeSeek(std::ifstream& disk_file, uint64_t seek_location) {
...
void DiskProver::SafeRead(std::ifstream& disk_file, uint8_t* target, uint64_t size) {
...
// are looking for.
uint128_t DiskProver::ReadLinePoint(std::ifstream& disk_file, uint8_t table_index, uint64_t position)
{
...
std::vector<uint64_t> DiskProver::GetP7Positions(
    uint64_t curr_f7,
    uint64_t f7,
...
std::vector<uint64_t> DiskProver::GetP7Entries(std::ifstream& disk_file, const uint8_t* challenge)
{
    if (C2.empty()) {
        return std::vector<uint64_t>();
    }
...
std::vector<LargeBits> DiskProver::ReorderProof(const std::vector<Bits>& xs_input) const
{
...
std::vector<Bits> DiskProver::GetInputs(uint64_t position, uint8_t depth, std::ifstream* disk_file = nullptr)
{
